...
\maketitle 
\begin{abstract}
  \todo{Mention corresponding author}
...
For this scenario, we achieve high-order convergence (\ref{fig:convergence-test}) but notice some diminishing returns for large orders\todo{Maybe note that Dumbser doesnt get perfect convergence either.}.
\begin{figure}[tb]
  \centering
...
\begin{minipage}[t]{.473\textwidth}
  \centering
    ...
    Colliding bubbles with \muscl{}. Contour values for potential temperature perturbation are $-0.05, 0.05, 0.1, \ldots 0.45$. TODO UPDATE!}
\end{minipage}
\end{figure}
...
We can see~\pcref{fig:two-bubbles-ader} that the lower resolution leads to spurious wave and does not capure the behavior of the cloud.
...
Using a single MPI rank, we have roughly $4.9$ millions degrees of freedom updates ($\mdofs$) using two TBB threads and $20.15 \mdofs$ using 24 threads (i.e.\ a half node).
...
We further note that for our scenarios weak scaling is more important than strong scaling, as we currently cover only a small area containing a single cloud, where in practical applications one would like to smilate more complex scenarios.
\section{Conclusion}
...
As mentioned in \cref{sec:results}, the resolution is sufficient for most numerical weather prediction purposes\todo{Actually mention this}.
