...
\maketitle 
\begin{abstract}
  We present an application of the compressible Navier Stokes equations for cloud formation processes.
...
We use the fact that the total variation of a numerical solution is a perfect indicator for edges of a wavefront.
...
\begin{equation}
  ...
  \begin{cases}
    ...
    \end{cases}
\end{equation}
The parameters $\Trefine > \Tdelete$ can be chosen freely\todo{LK: Do we need the ineq.?}.
...
A comparison at time $t = 10.0$ of the analytical solution for the pressure with our approximation (figure xxx) shows an excellent agreement.\todo{fig, check time, fix order/cells.}
...
\begin{figure}[htb]
  ...
  \begin{subfigure}[t]{0.5\textwidth}
    ...
    \caption{\label{fig:cavity-flow}Our approximation (lines) of the lid-driven cavity flow vs.\ reference solution (markers) of~\cite{ghia1982high}.}
  \end{subfigure}~%
  \begin{subfigure}[t]{0.5\textwidth}
    \centering
...
The Arnold-Beltrami-Childress (\textsc{abc}) flow is similar to the Taylor-Green vortex but posesses an analytical solution for three-dimensions as well~\cite{tavelli2016staggered}.
It is defined in the domain \( \left[ -\SI{\pi}{\m}, \SI{\pi}{\m} \right]^3 \) as
\begin{align}
  \label{eq:abc-flow}
  \begin{split}
  \Qrho(\bm{x}, t) &= 1,\\
...
\begin{minipage}{.5\textwidth}
  \centering
    \includegraphics{paper_abc_flow_velocity}
    \captionof{figure}{\label{fig:abc-flow}Our approximation (lines) of the lid-driven cavity flow vs.\ reference solution (markers) of~\cite{ghia1982high}.}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
...
Our stratified flow scenarios are described in terms of the potential temperature
\begin{equation}
...
\begin{equation}
  \label{eq:radius}
  r^2 = \Vert \bm{x} - \bm{x_c} \Vert_2 %(x - x_0)^2 + (z - z_0)^2
\end{equation}
denote the Euclidian distance between the spatial positions $\bm{x} = x, z$ and the center of a bubble $\bm{x_c} = x_c, z_c$.
In case of three-dimensional scenarios the vectors $\bm{x}$ and $\bm{x_c}$ also contain a y coordinate.
...
We ran this scenario twice, firstly without \amr{} and a mesh of size $\SI{1000/81}{\m} = \SI{12.35}{\m}$.
For the second run we used \amr{} with two adaptive refinement levels and parameters $\Trefine = 2.5$ and $\Tdelete = -0.5$.
We specialize the \amr{}-criterion~\pcref{eq:refinement-criterion} to our stratified flows by using the potential temperature.
This resulted in a mesh with cell sizes lengths of ca. $\SI{111.1}{\m}, \SI{37.04}{\m}, \SI{12.34}{\m}$.
The resulting mesh can be observed in \cref{fig:two-bubbles-ader}.
We observed that even though both results are nearly identical, the \amr{} run managed to use only 1953 cells, instead of 6561 cells for the fully refined solution. 
% points: 70308, 23619
We further want to emphasize that our \amr{}-criterion is able to accurately track the position of the edges of cloud instead of only its position.
...
Overall, our result for this benchmark shows an excellent agreement to the previous solutions of~\cite{muller2010adaptive,giraldo2008study}\todo{Is coll.\ bubble contained in se study?}.

In addition, we simulated the same scenario with our \muscl{} method, using $3^2$ patches with $80^2$ finite volume cells each~\todo{check}.
As we use limiting, we do not need any viscosity.
The results of this method (\cref{fig:two-bubbles-fv}) also agree with the reference.
Note that the numerical dissapitivity of the finite volume scheme has a smoothing effect that is similar to the smoothing caused by viscosity.

For our second scenario, the cosine bubble, we use a pertubation of the form
\begin{align}
...
This corresponds to the parameters used in~\cite{kelly2012continuous}.
For the 2D case, we use a constant viscosity of $\mu = 0.01$, an \aderdg{}-method of order 5 with two levels of dynamic \amr{}, resulting again in cell sizes of roughly $\SI{111.1}{\m}, \SI{37.04}{\m}, \SI{12.34}{\m}$.
...
This result shows an excellent agreement to the ones achieved in~\cite{giraldo2008study}\todo{correct source?}.
...
We again use a constant viscosity of $\mu = 0.01$ for the 2D case, and a viscosity of $\mu = 0.02$ for the 3D case.
The simulations run for a time of $\SI{600}{\s}$ and $\SI{400}{\s}$ for two and three-dimensional scenarios respectively.
...
\section{Conclusion}
We presented our implementation of an \aderdg{}-method and a \muscl{}-scheme with \amr{} for the Navier-Stokes equations.
...
Leonhard Rannabauer was funded by the European Union’s Horizon 2020 Research and Innovation Programme under the project ExaHyPE, grant no.  671698 (call FETHPC-1-2014)
Lukas Krenz  the European Union’s Horizon 2020 Research and Innovation Programme under the project ChEESE, grant no.\ 823844.
Special thanks goes to Dominic E.\ Charrier who provided help with the implementation in the \exahypeengine{}.
\todo{Are those ack.\ correct, esp. call no.\ for Cheese? Also thank lrz?}
