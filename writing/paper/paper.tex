...
\end{equation}
and relates to the temperature $T$ by the thermal equation of state
\begin{equation}
...
\end{equation}
i.e.\ a state where the pressure differences are exactly balanced with the gravitational source term.
The momentum equation is dominated by the background flow in this case.
This leads to numerical instabilities.
To avoid this, we split the pressure $\pressure = \backgroundPressure + \pressure'$ and density $\Qrho = \backgroundRho + \Qrho'$, arriving at the modified equation
\begin{equation}
...
As a final example of standard flow scenarios, we consider the lid-driven cavity flow which initial conditions are given by a fluid at rest, with $\Qrho = 1, \pressure(\bm{x}) = \nicefrac{100}{\gamma}$.
...
\end{equation}
with reference pressure $p_o = \SI{10e5}{\Pa}$~\cite{muller2010adaptive,giraldo2008study}.
For all remaining scenarions, we use the constants
\begin{equation}\label{eq:atmosphere-constants}
    \gamma = 1.4 ,\qquad \Pr =  0.71 ,\qquad R = 287.058 ,\qquad p_0 = 10000, \qquad g = 9.8.
\end{equation}
