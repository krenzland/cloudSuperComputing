...
\begin{abstract}
 ...
 \todo{Abstract---ideas for last two sentences. Note that adding one sentence leads to more pages.}
% 
...
For this scenario, we achieve high-order convergence (\cref{fig:convergence-test}) but notice some diminishing returns for large orders\todo{Here a sentence regarding convergence would be nice---Dumbser got better results}.
\begin{figure}[tb]
  \centering
...
We ran the two bubble scenario for a uniform grid with a mesh size of $729 \times 729$ with order 6, resulting in roughly 104 million degrees of freedom (\textsc{dof}), for 20 timesteps and for multiple combinations of \mpi{} ranks and \tbb{} threads\todo{Check number of dofs? Should be $(6+1)^2 * 729^2$, right?}.
