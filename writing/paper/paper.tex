...
The \exahype{} project (\url{www.exahype.eu}) develops an engine to solve arbitrary hyperbolic partial differential equations (\pde{}s).
...
The underlying ADER-DG method, parallelisation techniques and dynamic adaptive mesh refinement are available for simulation while the implementations are left as a black box to the user.
...
In this paper we extended this generic approach for hyperbolic \pde{}s by parabolic terms.
We base this extension on the work of \citeauthor{gassner2008discontinuous} \cite{gassner2008discontinuous}, which has already been applied to the ADER-DG method in \cite{dumbser2010arbitrary}.
%Including these terms gives users the option to solve arbitrary parabolic \pde{}s \todo{LR: stimmt das ?}.
We want to emphasize that with this extension we add the option to limit the numerical scheme \todo{LR: ref Ja/Nein?} with viscosity terms. This will exploited in this paper in a series of scenarios for the compressible Navier-Stokes equations which implicitly include viscosity as part of the equation.
...
\section{Methods}
\todo{Write \aderdg{}/\exahype{} section! Riemann solver + timestep. LR: Was sollen wir da deiner Meinung nach noch erwähnen?

LK: CFL Condition ist wictig, wg.\ skalierung, Riemann solver weil wir eben nicht mixed-elements machen und keinen gradienten speichern müssen. Außerdem mehr oder weniger general purpose flux.
}
...
The user can state dynamic \amr{} rules by supplying custom criteria that are evaluated pointwise.
...
We compute the integral efficiently with Gaussian quadrature over the collocated quadrature points.\todo{If using muscl, add sentence about finite differences estimate}
\newcommand{\mean}{\mu}%
\newcommand{\std}{\sigma}%
