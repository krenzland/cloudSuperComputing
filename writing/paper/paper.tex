...
For the \textsc{cfd} scenarios, we use the following constants of fluids:
\begin{equation}
...
A comparison at time $t = 1.0$ of the analytical solution for the pressure with our approximation (figure xxx) shows an excellent agreement.\todo{fig, check time, fix order/cells.}
We used an \aderdg{}-scheme of order $5$ with a grid of $25^2$ cells.

The Arnold-Beltrami-Childress (\textsc{abc}) flow is similar to the Taylor-Green vortex but posses an analyitcal solution fir three-dimensions as well~\cite{tavelli2016staggered}.
...
Figure xxx show a good agreement between the analytical solution and our approximation with an \aderdg{}-scheme of order $2$ with a mesh consisting of $27^3$ cells at time $t = \SI{0.1}{\s}$\todo{Figure}.
...
Again, our results have an excellent agreement with the reference solution of~\cite{ghia1982high}.
We used an \aderdg{}-method of order $3$ with a mesh of size $27^2$\todo{check!}.

Our stratified flow scenarios are described in terms of the potential energy
\begin{equation}
...
We use a constant viscosity of $\mu = 0.1$ and a simulation time of $\SI{400}{\s}$ for 3d and $\SI{600}{\s}$ for 2d.
...
To ensure that the atmosphere stays in hydrostatic balance, we need to impose the viscous heat flux
\begin{equation}
...
\end{equation}
where $\overline{T}(z)$ is the background temperature at position $z$ which can be computed from \cref{eq:hydrostatic-balance,eq:eos}~\cite{giraldo2008study}.
