...
\begin{equation}
  ...
  -g \bm{k} \Qrho'.
\end{equation}
\todo{Note that similar splitting is performed in giraldo}
\section{Numerics}
...
In addition, we use a \muscl{} finite volume method with the same flux and the van Albada \todo{LK: Which one is best? Cite vanalbada} limiter.
...
\end{equation}
with probability $\mathbb{P}$ guarantees that we neither refine nor delete all cells.\todo{Mention that all cells of a smaller level need to be \textbf{marked} as erase to trigger coarsening.}
