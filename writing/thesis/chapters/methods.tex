...
We define basis functions on the reference cell.
...
\begin{equation}
  ...
  f(x) = \sum_{i = 0}^N f(x_i) \lagrange[i](x) = f(x_i) \lagrange[i](x),
\end{equation}
...
\begin{equation}
  ...
  f(\bm{x}) = \sum_{i = 0}^{N^d} f(\bm{x}_i) \lagrange[i](\bm{x}) = f(\bm{x}_i) \lagrange[i](\bm{x}).
\end{equation}
Similarly to the one-dimensional case, we can evaluate integrals by
\begin{align}
  \label{eq:quadrature}
  \int_0^1 f(\bm{x}) \dd{\bm{x}} &= \sum_n^{(N+1)^D - 1} \quadWeight[n] f(\bm{x}_n) \\
  \intertext{with}
...
&& \text{Interpolating:} \qquad & \lagrange[i] (x_j) &= \delta_{ij}\\
\label{eq:basis-orthogonality}
&& \text{Orthogonality:} \qquad & \ldots &= \ldots
    \end{alignat}
...
\sidetitle{Corrector}
First, we multiply the system \cref{eq:conservation-law-gradient} by a test function $\testfunction{i}$ and integrate over the space-time volume $(\cell \times [t^n, t^{n+1}])$.
Note that the testfunctions
\begin{equation}
  \label{eq:cell-approx-space}
  q(\bm{x})_{|\cell[i]} = \hat{u}^n_{i,l} \stbasis{l}(\bm{x}).
\end{equation}
