...
The first row is the conservation of mass, the second is the conservation of momentum, the third is the conservation of energy and the fourth is the continuity of the gas.
...
It is useful to compute time-averaged versions of our quantities directly after the predictor.
...
\end{equation}
We now insert our discrete approximations and apply the basis expansions~\pcref{eq:cell-approx-space,eq:cell-approx-space-time} to the space-time predictor.
...
Using the orthogonality of the basis functions~\pcref{eq:basis-orthogonality} on the first term, it becomes clear that the correction is a simple, explicit computation.
...
\todo{Should i mention paper only describe for 1D?}
...
\begin{equation}\label{eq:cfl-aderdg}
 ...
 \Delta t \leq  \text{CFL} \, \alpha(N) \, h \sum_{i=1}^{\dimensions} \frac{1}{\maxConvEigen + 2 \maxViscEigen \frac{1}{\alpha(N) h}},
\end{equation}
...
We tread the source term by a simple splitting scheme.
...
If a solution violates any of our criterions, we recompute the last timestep for the offending cells with a limited finite volume method.
