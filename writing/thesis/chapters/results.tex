...
\begin{figure}[htb]
  ...
  \begin{subfigure}[t]{0.5\textwidth}
    \centering
    \includegraphics[]{thesis_lid_driven_cavity_stream}
...
Note that the \amr{} criterion manages to track the bubble correctly~(\cref{fig:two-bubbles-amr-grid}) and introduces only small errors~(\cref{fig:two-bubbles-amr-error}) compared to the solution with the fully refined mesh\todo{Increase font size a bit more}.
