...
\begin{itemize}
\item Generate glue-code using a \softwareName{Python}-based Toolkit that inserts parameters into templates.\todo{Jinja? Or sth else?}
...
\item The user then needs to modify the generated glue code.
  ...
  We describe the grid refinement criterion in \funName{refinementCriterion} and the limiting criterion in \funName{isPhysicallyAdmissible}
...
The information for each cell is defined in the file \fileName{ADERDGCellDescrition.def} for the \dg{}-solver.
...
\item Modifying the space-time-predictor (defined in file \fileName{spaceTimePredictorNonlinear.cpph}).
  \begin{itemize}
  \item In the function \funName{aderPicardLoopNonlinear} we pass the gradient of the conserved variables to the \funName{flux} method.
    We also return the time-average of the gradient.
   \item In the function \funName{aderExtrapolatorNonlinear} we also compute the extrapolated gradient.
...
\item Adding the diffusive penalty terms for Riemann solver (\fileName{riemannSolverNonlinear.cpph}) and \textsc{cfl}-condition (\fileName{stableTimeStepSize.cpph}).
...
We use three functions that need to be defined by the user and adher to the interface
\begin{itemize}
\item The function \funName{resetGlobalObservables} takes no arguments and returns a vector of size $\ngobs$.
  ...
  \todo{Is this right?}
\item The function \funName{mapGlobalObservables} that takes the degrees of freedoms of a cell and its cell size and returns a vector of size $\ngobs$.
...
\begin{algorithm}[htb]
  \begin{algorithmic}
    ...
    \Let{curObservables}{\Call{mapGlobalObservables}{$Q_\text{cell}, \Delta x_\text{cell}$}}
...
\todo{Use Q, TV macro here!}
The vector $(-1, -1, 0)$ is then the base-case for the iteration (\funName{resetGlobalObservables}).
The method \funName{mapGlobalObservables} is defined to return $\left( \operatorname{TV} \left[ f(\bm{Q}) \right], 0, 1 \right)$, where $f(\bm{Q})$ returns an arbitary indicator value per cell.
Finally, \funName{reduceGlobalObservables} is implemented exactly as described in \cref{alg:merge-variance}.

For the actual implementation we follow \cref{alg:reduce-gobs} but include nodal and intra-nodal parallelization in the following way:
\begin{itemize}
\item Add vectors \varName{\_globalObservables} and \varName{\_nextGlobalObservables} to all solvers.
  ...
  At the beginning of each timestep, we overwrite the previous timestep data with the current data, and reset \varName{\_nextGlobalObservables} with \funName{resetGlobalObservables}.
  This is done in the methods \funName{startNewTimeStep}, \funName{startNewTimeStepFused}, \funName{updateTimeStepSizesFused}, \funName{updateTimeStepSizes}.
\item Reduce global observables in solver with method \funName{reduceGlobalObservables} that takes \varName{cellInfo} and \varName{solverNumber}.
...
\item Reduce global observables per node in the mappings \className{FinaliseMashRefinement}, \className{FusedTimeStep}, \className{UpdateAndReduce}.
  \begin{itemize}
  ...
  \item If we use \tbb{}, we merge the partially reduced observables in \funName{mergeWithWorkerThread} with \funName{reduceGlobalObservables} with two vectors as arguments.
    This is not necessary for the mapping \className{FinaliseMeshRefinement}.
   \item We update the global observables of the solver in the method \funName{endIteration}.
  \end{itemize}
\item If we use \mpi{}, we also need to reduce the observables over all ranks.
  ...
  For this, we changed the following methods\todo{Next and current?!} of the \dg{}-solver:
  \begin{itemize}
  ...
  \end{itemize}
  This reduction is not supported for the limiting and finite volumes solver.\todo{Support for limiting?}
